...
UniFi2MQTT 4.0.3 keeps the official Network Integration API authoritative for discovery, device identity, port inventory, link state, negotiated speed, connector and PoE data, then adds non-fatal per-port traffic enrichment from the read-only classic Network `stat/device` endpoint.
...
The activity path was live-tested on a USW Flex Mini (`USMINI`) through both Local and Remote APIs, including moving per-port counters on a 1 Gbit/s link. The 4.0.3 runtime default is 10-second polling, aligned with Switch Vision Core's 12-second activity hold.
