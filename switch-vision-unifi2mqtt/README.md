# Switch Vision UniFi2MQTT v2.0.44
...
Required UniFi settings are `controller_url` and `api_key`. `site_id` defaults to `auto`; the bridge queries the local UniFi Network site list and resolves the actual site UUID automatically. Multi-site users may instead configure a site UUID, exact site name, or internal reference. Home Assistant's Supervisor MQTT service is used automatically unless a custom `mqtt_host` is configured. The API key is sent only in the `X-API-KEY` request header. New installations verify controller TLS certificates by default. A single empty switching-device response no longer retires all devices; whole-set retirement requires three consecutive empty polls. No write/action endpoints are used.
